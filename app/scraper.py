...
def scrap_stackoverflow_pg():
    for pg in range(1, 3):
        Job.objects.all().filter(automated=True).delete()
        Tag.objects.all().delete()
        connection.close()

        URL = "https://stackoverflow.com/jobs?dr=FullStackDeveloper?pg={}".format(pg)
        page = requests.get(URL)

        soup = BeautifulSoup(page.content, 'html.parser')
        jobs_list = soup.find(class_='listResults')

        jobs_items = jobs_list.find_all(class_='fl1')   
        jobs_image_items = jobs_list.find_all('img', class_='s-avatar--image')
        ...
        for i, job_element in enumerate(jobs_items):
            if i == len(company_images):
                ...
                job_content = str(job_soup.find(class_='mb32 fs-body2 fc-medium'))
                ...
                tags_element = job_element.find_all('a', class_='s-tag')
